...
## 18. Magapoke M3c1/M3c2 and Catalog schema v4

Catalog schema v4 adds nullable `sources.published_at` as Source-specific observed publication metadata. Date-only Magapoke observations use the live-confirmed `YYYY/MM/DD` format and normalize to aware JST midnight for stable ordering. Missing or malformed dates do not make Discovery incomplete. The field never belongs to Item and never becomes `order_key`.
...
Magapoke Batch planning places direct candidates first. A quota source with a future observed grant uses direct access without consuming a new resource. The first quota pass offers at most one `work_ticket` candidate per Work, selected by oldest `Source.published_at` first, NULL last, stable item ordering, then `source_id DESC` for remaining ties. Work order is `Work.created_at`, then `Work.id`. After every direct and Work Ticket candidate has been attempted, Batch rereads Catalog and builds the Premium pass from still-pending quota sources. Premium candidates use `quota_resource="premium_ticket"`, `quota_scope="work"`, no per-Work limit, and the same oldest-date/stable tie ordering; this groups all selected episodes in one Work before moving to the next older Work. Premium is account-global and is never stored in Catalog. Manga ONE and BookWalker retain their existing site-wide quota allocation.

Jump+ Batch uses the same Policy-owned Work ordering for normal direct candidates: `published_at ASC`, NULL last, and `source_id DESC` for same-date ties. Jump+ Discovery remains latest-first; only the subsequent Batch candidate order is reversed by publication date within each Work. Jump+ has no supported automatic rental/resource pass, so this does not change access eligibility or resource consumption.

Site Policy declares ordered additional resource passes; the generic Planner does not interpret resource names. `PolicyDecision` carries site-neutral `quota_resource`, `quota_scope`, `quota_limit`, and `quota_commit_mode` metadata. Both Magapoke ticket resources use `after_observed_consumption`. The Adapter accepts only `work_ticket` and `premium_ticket` for quota, and keeps separate exact-control entry paths. Premium count is read immediately before its click from exactly one semantic `dl.p-episode-purchase__point` whose normalized label is `プレミアムチケット` and value matches `N枚`. Header counts are diagnostic only. Missing, duplicate, or malformed balance data and unknown/ambiguous purchase actions fail closed. A zero balance never clicks and stops the Premium pass. A Work Ticket-only screen while Premium is requested is an expected `premium_ticket_unavailable` skip. Neither resource falls back to the other; points, coins, subscriptions, and paid purchase controls are never clicked.

The Adapter reports `AccessConsumption` only after the requested ticket control disappears and viewer canvas content is observed. The Executor persists `quota_started_at=consumed_at` and the conservative Magapoke grant `consumed_at + 71 hours` before packaging, including when crawling then fails. Unavailable, exhausted, and already-accessible cases leave quota state unchanged. This 71-hour grant is verified for Work Ticket by the successful M3c1 live run; applying the same grant to Premium Ticket remains an assumption until its live rental state is independently verified. Premium balance is re-read from the page for every candidate and is not persisted; sequential execution plus the zero-balance stop prevents further Premium attempts once exhaustion is observed.
