...
特定のWebビューアにアクセスし、現在コンテンツを1ページずつ進めながら本文だけを画像artifactとして保存する。
...
Detailed capture-selection rules are defined in `docs/CAPTURE_STRATEGY.md` and that document is the authority for capture priority and renderer-safety policy.
...
A higher level is used only when it can be attributed safely to the visible page and does not destabilize the viewer. File-format uniformity is not a reason to re-encode a correct original artifact. Expensive encoding or hashing must not be placed in the renderer's draw-time critical path; mutable sources may require a bounded draw-time pixel snapshot, but encoding should be deferred until after the selected draw call is known.
...
source-native or adapter-generated bytes with `mime_type` and `file_extension`
(currently `.jpg`, `.webp`, and `.png` are supported by packaging). Runner filenames, dimensions, and
...
fallback後の通常capture失敗は通常どおりrun errorにする。BookWalkerはsource-native PNG/JPEGを
...
It accepts safe generated PNG/JPEG/WebP paths, rejects missing or unsafe entries,
and does not include unlisted files. A run may therefore contain mixed
source-native/adapter JPEG or WebP and screenshot PNG artifacts.

正常な `END` / `NEXT_CONTENT` 後、manifestの `pages[].file` をauthorityとしてZIPを作成する。

- manifestにない画像artifactをZIPへ入れない
- manifest記載画像artifactが欠落していれば失敗する
- path traversal等の危険なmanifest pathを拒否する
- 完成ZIPはlibrary treeへ配置する
- completion statusを別途保存する
- 中間crawl directoryは、内容がmanifest / progress / manifest記載画像artifactだけの場合に限り削除する
- 無関係ファイルや余分な画像artifactがあればdirectory全体を削除しない
...
- UNKNOWN
- `max_pages`境界
- packagingで余分な画像artifactが混入しないこと
